...
def rule_score(item):
    ...
    if any(b in (item.get("source", "").lower()) for b in boosts):
        score += 2
    return score
...
def ai_refine(items):
    ...
    try:
        ...
        refined = []
        for it in items:
            prompt = f"""你將只輸出 JSON，且內容極簡。

請針對下列 AI 新聞依規則給出 1~5 分四項評分（tech/impact/practical/timely）：
標題：{it['title']}
摘要：{it.get('summary','')}
來源：{it.get('source','')}
...
輸出 JSON：{{"tech":x,"impact":x,"practical":x,"timely":x}}"""
            ...
            try:
                ...
                s = (
                    ...
                    + int(data.get("practical", 0))
                    + int(data.get("timely", 0))
                )
                refined.append(s)
            except Exception:
                refined.append(None)
...
def generate_formats(selected):
    ...
    for it in selected:
        date = to_display_date(it["published_at"])
        lines.append(
            f"- {it.get('source','未知機構')}（{date[:4]}）。{it['title']}。取自 {it['url']}"
        )
    fmt_b = "\n".join(lines)
    lines = []
    for it in selected:
        date = to_display_date(it["published_at"])
        lines.append(
            f"{it['title']}｜AI, Tech｜{it.get('source','')}｜{date}｜{it['url']}"
        )
    fmt_c = "\n".join(lines)
