...
def main():
    ...
    trend = []; latest = []; topics = Counter()
    for dt, sel in date_dirs_in_range(start, now):
        items = read_json(sel, default=[])
        trend.append({"date": dt.isoformat(), "count": len(items)})
        for it in items:
            latest.append({"title": it["title"], "url": it["url"], "source": it.get("source",""), "date": to_display_date(it["published_at"])})
    ...
    write_json(f"analysis/weekly/{iso_week}.json", {"generated_at": now.isoformat(),"range":[start.isoformat(), now.isoformat()],"hot_topics":hot_topics,"trend":trend,"latest":latest[-50:]})
    write_json("docs/api.json", {"hot_topics": hot_topics[:20], "trend": trend, "latest": list(reversed(latest))[:50]})
